...
class WarrenFactory(val server: ServerConfiguration, val user: UserConfiguration, val channels: ChannelsConfiguration, val events: EventConfiguration) {
    ...
    fun create(): IrcConnection {
        ...
        val capState = CapState(lifecycle = capLifecycleState, negotiate = setOf(CapKeys.CAP_NOTIFY.key, CapKeys.MULTI_PREFIX.key, CapKeys.SASL.key, CapKeys.ACCOUNT_NOTIFY.key, CapKeys.AWAY_NOTIFY.key, CapKeys.EXTENDED_JOIN.key, CapKeys.USERHOST_IN_NAMES.key), server = mapOf(), accepted = setOf(), rejected = setOf())
